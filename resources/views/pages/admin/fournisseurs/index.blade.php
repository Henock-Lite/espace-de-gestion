<x-layout.admin title="Fournisseurs">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
            <h1 class="text-xl md:text-2xl font-bold">Fournisseurs</h1>
            <p class="text-muted-foreground text-sm">Gérer les fournisseurs de la pharmacie</p>
        </div>
        <a href="{{ route('fournisseurs.create') }}" class="btn self-start sm:self-auto">+ Nouveau fournisseur</a>
    </div>
                ...
                <tbody class="divide-y divide-border">
                    @forelse($fournisseurs as $fournisseur)
                        <tr class="hover:bg-border/20 transition">
                            <td class="px-4 py-3 text-muted-foreground hidden sm:table-cell">{{ $fournisseur->id }}</td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex gap-1 md:gap-2">
                                    ...
                                    <x-modal-confirm :action="route('fournisseurs.destroy', $fournisseur->id)" message="Désactiver ce fournisseur ?" title="Désactiver le fournisseur"/>
                                </div>
                            </td>
                        </tr>
                    @empty
