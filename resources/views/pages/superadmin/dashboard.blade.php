<x-layout.superadmin title="Super Admin Dashboard">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold">Dashboard Global</h1>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-card border border-border p-4 rounded-lg">
            <p class="text-muted-foreground text-sm">Utilisateurs actifs</p>
            <p class="text-2xl font-bold">{{ $usersActifs }}</p>
            <p class="text-xs text-muted-foreground mt-1">{{ $totalUsers }} au total</p>
        </div>

        <div class="bg-card border border-border p-4 rounded-lg">
            <p class="text-muted-foreground text-sm">Ventes totales</p>
            <p class="text-2xl font-bold">{{ $ventesTotales }}</p>
        </div>

        <div class="bg-card border border-border p-4 rounded-lg">
            <p class="text-muted-foreground text-sm">CA ce mois</p>
        ...
        <div class="bg-card border border-border p-4 rounded-lg">
            <p class="text-muted-foreground text-sm">CA aujourd'hui</p>
            <p class="text-2xl font-bold text-primary">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-card border border-border p-4 rounded-lg">
            <p class="text-muted-foreground text-sm">Lots périmés</p>
            <p class="text-2xl font-bold {{ $lotsPerimes > 0 ? 'text-red-400' : '' }}">
                {{ $lotsPerimes }}
            </p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="bg-card border border-border rounded-lg overflow-hidden">
            <div class="px-4 py-3 border-b border-border">
                <h2 class="font-semibold">Produits les plus vendus</h2>
            </div>
