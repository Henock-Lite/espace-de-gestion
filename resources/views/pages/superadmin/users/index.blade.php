                        ...
                        <tr class="hover:bg-border/20 transition">
                            ...
                            <td class="px-4 py-3">
                                @if($user->role === 'super_admin')
                                    <span class="px-2 py-1 bg-purple-500/10 text-purple-400 rounded text-xs font-medium">Super Admin</span>
                                @else
                                    <span class="px-2 py-1 bg-blue-500/10 text-blue-400 rounded text-xs font-medium">Admin</span>
                                @endif
                            </td>
                            <td class="px-4 py-3">
                                @if($user->is_active)
                                    <span class="px-2 py-1 bg-green-500/10 text-green-400 rounded text-xs font-medium">Actif</span>
                                @else
                                    <span class="px-2 py-1 bg-red-500/10 text-red-400 rounded text-xs font-medium">Inactif</span>
                                @endif
                            </td>
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    @if($user->id !== auth()->id())
                                        <form method="POST" action="{{ route('users.destroy', $user->id) }}"
                                            ...
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-xs px-3 py-1 bg-red-500/10 text-red-400 border border-red-500/20 rounded hover:bg-red-500/20 transition">
                                                Désactiver
                                            </button>
                                        </form>
                                    @endif
